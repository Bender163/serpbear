...
const exportCSV = (keywords: KeywordType[] | SCKeywordType[], domain:string, scDataDuration = 'lastThreeDays') => {
   ...
   let csvHeader = 'ID,Keyword,Position,URL,Country,City,Device,Engine,Updated,Added,Tags\r\n';
   let csvBody = '';
   ...
   if (isSCKeywords) {
      ...
      keywords.forEach((keywordData) => {
         const { ID, keyword, position, url, country, city, device, lastUpdated, added, tags, engine } = keywordData as KeywordType;
         // eslint-disable-next-line max-len
         csvBody += `${ID}, ${keyword}, ${position === 0 ? '-' : position}, ${url || '-'}, ${countries[country][0]}, ${city || '-'}, ${device}, ${engine || 'google'}, ${lastUpdated}, ${added}, ${tags.join(',')}\r\n`;
      });
   }
